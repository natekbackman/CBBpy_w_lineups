...
def _get_game_boxscore_helper(boxscore, game_id):
    ...
    if len(tm1_starters) > 0:
        ...
        tm1_st_nm = [
            (
                tm1_starters[i]["athlt"]["dspNm"]
                if "dspNm" in tm1_starters[i]["athlt"].keys()
                else ""
            )
            for i in range(len(tm1_starters))
        ]
    ...
    if len(tm1_bench) > 0:
        ...
        tm1_bn_nm = [
            (
                tm1_bench[i]["athlt"]["dspNm"]
                if "dspNm" in tm1_bench[i]["athlt"].keys()
                else ""
            )
            for i in range(len(tm1_bench))
        ]
    ...
    if len(tm2_starters) > 0:
        ...
        tm2_st_nm = [
            (
                tm2_starters[i]["athlt"]["dspNm"]
                if "dspNm" in tm2_starters[i]["athlt"].keys()
                else ""
            )
            for i in range(len(tm2_starters))
        ]
    ...
    if len(tm2_bench) > 0:
        ...
        tm2_bn_nm = [
            (
                tm2_bench[i]["athlt"]["dspNm"]
                if "dspNm" in tm2_bench[i]["athlt"].keys()
                else ""
            )
            for i in range(len(tm2_bench))
        ]
...
def _get_game_pbp_helper(gamepackage, game_id, game_type):
    ...
    df['is_score'] = play_lower.str.contains('makes|made', regex=False, na=False)
    df['is_miss'] = play_lower.str.contains('misses|missed', regex=False, na=False)
    df['is_fga'] = play_lower.str.contains('makes|made|misses|missed', regex=False, na=False)
